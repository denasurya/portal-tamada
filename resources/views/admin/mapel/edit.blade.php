...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800">Edit Mapel: {{ $mapel->nama }}</h1>
</div>
    ...
    <form action="{{ route('admin.mapel.update', $mapel->id) }}" method="POST" class="p-6">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="kode" id="kode" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('kode') border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500 @enderror" value="{{ old('kode', $mapel->kode) }}" required autofocus>
                @error('kode')
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('nama') border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500 @enderror" value="{{ old('nama', $mapel->nama) }}" required>
                @error('nama')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>

...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800">Tambah Jurusan Baru</h1>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-2xl">
    <form action="{{ route('admin.jurusan.store') }}" method="POST" class="p-6">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nama" id="nama" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('nama') border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500 @enderror" value="{{ old('nama') }}" required autofocus>
            @error('nama')

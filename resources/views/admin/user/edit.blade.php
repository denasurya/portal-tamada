...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800">Edit User: {{ $user->username }}</h1>
</div>
    ...
    <form action="{{ route('admin.user.update', $user->id) }}" method="POST" class="p-6">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="nama_lengkap" id="nama_lengkap" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('nama_lengkap') border-red-300 @enderror" value="{{ old('nama_lengkap', $user->nama_lengkap) }}" placeholder="Kosongkan jika ingin memakai username sebagai nama">
            ...
            <div>
                ...
                <input type="text" name="username" id="username" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('username') border-red-300 @enderror" value="{{ old('username', $user->username) }}" required autofocus>
            ...
            <div>
                ...
                <select name="role" id="role" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('role') border-red-300 @enderror" required {{ auth()->id() === $user->id ? 'disabled' : '' }}>
                    <option value="admin" {{ old('role', $user->role) == 'admin' ? 'selected' : '' }}>Admin (Akses Penuh)</option>
                    <option value="kepsek" {{ old('role', $user->role) == 'kepsek' ? 'selected' : '' }}>Kepala Sekolah (Hanya Laporan)</option>
                </select>
                @if(auth()->id() === $user->id)
                    <input type="hidden" name="role" value="{{ $user->role }}">
                    <p class="text-xs text-gray-500 mt-1">Anda tidak dapat mengubah role akun sendiri.</p>
                @endif

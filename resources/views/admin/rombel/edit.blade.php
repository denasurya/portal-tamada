...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800">Edit Rombel: {{ $rombel->nama }}</h1>
</div>
    ...
    <form action="{{ route('admin.rombel.update', $rombel->id) }}" method="POST" class="p-6">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="nama" id="nama" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('nama') border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500 @enderror" value="{{ old('nama', $rombel->nama) }}" required autofocus>
                @error('nama')
